...
local function moveTo(method, destination, delayTime, tweenStyle, tweenDirection)
	cancelFlag = false
	...
	elseif method == "Tween" then
		...
		local tween = TweenService:Create(
			...
			TweenInfo.new(
				delayTime or 1,
				Enum.EasingStyle[tweenStyle or "Linear"],
				Enum.EasingDirection[tweenDirection or "Out"]
			),
...
function ZerathTPGen:Teleport(config)
	safe(function()
		local method = config.Method or config.Teleport_Method
		local teleportType = config.Type or config.Teleport_Type or "Normal"
		local coordinates = config.Coordinates
		local delayTime = config.Delay or config.Teleport_Delay or 1
		local yOffset = config.OffsetY or config.Teleport_Type_Y or -50
		local cinematic = config.CinematicMode or false
		local tweenStyle = config.TweenStyle or config.Tween_Style
		local tweenDirection = config.TweenDirection or config.Tween_Direction
		...
		if teleportType == "Underground" then
			...
			moveTo(method, getOffsetPosition(coordinates, -math.abs(yOffset)), delayTime, tweenStyle, tweenDirection)
		...
		elseif teleportType == "Air" then
			...
			moveTo(method, coordinates, delayTime, tweenStyle, tweenDirection)
		end
